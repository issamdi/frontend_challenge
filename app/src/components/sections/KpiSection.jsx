...
import { LifeBuoy, Star, TrendingUp } from "lucide-react";
import { kpiStats } from "../../data/constants";

const ParticipantsIcon = ({ size = 26, ...props }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const KPI_ICONS = [TrendingUp, ParticipantsIcon, Star, LifeBuoy];

export default function KpiSection() {
  return (
    <SectionShell className="border-b border-slate-200 bg-white py-14">
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        {kpiStats.map(({ value, label }, index) => {
          const Icon = KPI_ICONS[index] ?? TrendingUp;

          return (
            <article key={value} className="text-center">
              ...
              <p className="text-[30px] font-bold leading-none text-brand-navy">
                {value}
              </p>
